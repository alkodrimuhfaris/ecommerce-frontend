...
export default function Profile(props) {
  ...
  const saveChange = (e) => {
    ...
    Object.values(dataForm).filter((item) => item).length &&
      dispatch(profileAction.patchProfile(token, dataForm));
      ...
      <Form className="my-3" onSubmit={saveChange}>
        <Row>
          <Col xs="12" md="8" className="mb-3">
            <FormGroup row>
              <Label className="text-right" for="name" sm={3}>
                Name
              </Label>
              <Col sm={9}>
                <Input
            ...
            <FormGroup check row>
              <Col sm={{size: 9, offset: 3}}>
                <Button color="success" type="submit" className="rounded-pill">
                  Submit
                </Button>
              </Col>
            </FormGroup>
          ...
          <Col xs="12" md="4">
            <Media className="flex-column align-items-center">
              <Media
                ...
                <Media
                  object
                  src={showAvatar || 'https://via.placeholder.com/130'}
