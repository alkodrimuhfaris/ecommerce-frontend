...
import { Container, Card, CardImg, CardTitle, CardSubtitle, CardText, Row, Col } from 'reactstrap';
...
class Home extends React.Component {
  ...
  render(){
    let newProducts = this.props.items.dataNewItems
    let popularProducts = this.props.items.dataPopularItems
    return(
      <React.Fragment>
        <NavBarClient />
        <Container>
                ...
                <Col xs='5' sm='4' md='3' lg='2'  className='m-2'>
                  <Link to={`/product/${product.id}`} style={{textDecoration: 'none', color: 'black'}}>
                    <Card className='shadoq'>
                      <CardImg top width='100%' src={this.imgProduct(product)} alt='Product image'/>
                      <Container className='my-3'>
                        ...
                        <CardSubtitle style={this.storeStyle} className='text-muted'>{product.store_name || 'Tuku Store'}</CardSubtitle>
                ...
                <Col xs='5' sm='4' md='3' lg='2'  className='m-2'>
                  <Link to={`/product/${product.id}`} style={{textDecoration: 'none', color: 'black'}}>
                    <Card className='shadow'>
                      <CardImg top width='100%' height='50%' src={this.imgProduct(product)} alt='Product image' />
                      <Container className='my-3'>
                        <div style={this.titleStyle}>{product.name}</div>
                        <CardSubtitle><strong className='text-success'>{currencyFormat(product.price)}</strong></CardSubtitle>
                        <CardSubtitle style={this.storeStyle} className='text-muted'>{product.store_name || 'Tuku Store'}</CardSubtitle>
